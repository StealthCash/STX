...
public:
    ...
    int64_t GetStealthCashBalance() const;
    ...
    int64_t GetStealthCashDebit(const CTxIn& txin) const;
    int64_t GetStealthCashCredit(const CTxOut& txout) const;
    ...
    int64_t GetDebit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txin.IsAnonInput())
            {
                nDebit += GetStealthCashDebit(txin);
            } else
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetStealthCashCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nSTX, int64_t& nStealthCash) const
    {
        nSTX = 0;
        nStealthCash = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nStealthCash += GetStealthCashCredit(txout);
            } else
                nSTX += GetCredit(txout);
            if (!MoneyRange(nSTX)
                || !MoneyRange(nStealthCash))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
    };
...
public:
    ...
    mutable bool fAvailableStealthCashCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableStealthCashCreditCached;
    
    mutable int64_t nCredSTXCached;
    mutable int64_t nCredStealthCashCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableStealthCashCreditCached = false;
        
        nCredSTXCached = 0;
        nCredStealthCashCached = 0;
        ...
        nAvailableStealthCashCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredSTX, int64_t& nCredStealthCash, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredSTX = nCredStealthCash = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredSTXCached = 0;
            nCredStealthCashCached = 0;
            pwallet->GetCredit(*this, nCredSTXCached, nCredStealthCashCached);
            fCreditSplitCached = true;
        };
        
        nCredSTX = nCredSTXCached;
        nCredStealthCash = nCredStealthCashCached;
        return true;
    }
    ...
    int64_t GetAvailableStealthCashCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableStealthCashCreditCached)
            return nAvailableStealthCashCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableStealthCashCredit() : value out of range");
            };
        };
        
        nAvailableStealthCashCreditCached = nCredit;
        fAvailableStealthCashCreditCached = true;
        return nCredit;
    };
