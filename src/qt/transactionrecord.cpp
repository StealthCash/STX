...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return StealthcashGUI::tr("Received with");
    case RecvFromOther:
        return StealthcashGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return StealthcashGUI::tr("Sent to");
    case SendToSelf:
        return StealthcashGUI::tr("Payment to yourself");
    case Generated:
        return StealthcashGUI::tr("Mined");
    case RecvStealthCash:
        return StealthcashGUI::tr("Received stealthcash");
    case SendStealthCash:
        return StealthcashGUI::tr("Sent stealthcash");
    case Other:
        return StealthcashGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvStealthCash:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendStealthCash:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredSTX, nCredStealthCash;
    wtx.GetCredit(nCredSTX, nCredStealthCash, true);
    int64_t nCredit = nCredSTX + nCredStealthCash;
    ...
    {
        if (nNet > 0 && nCredStealthCash > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvStealthCash, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendStealthCash, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvStealthCash;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv stealthcash
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendStealthCash;
                    sub.address = CBitcoinAddress(ckidD).ToString();
