#include "stealthcashbridge.h"

#include "stealthcashgui.h"
#include "guiutil.h"
...
#include "stealthcashbridge.moc"

StealthcashBridge::StealthcashBridge(StealthcashGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
StealthcashBridge::~StealthcashBridge()
{
...
void StealthcashBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void StealthcashBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void StealthcashBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void StealthcashBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void StealthcashBridge::paste() {
    emitPaste(QApplication::clipboard()->text());
}

// Options
void StealthcashBridge::populateOptions()
{
...
// Transactions
bool StealthcashBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void StealthcashBridge::clearRecipients()
{
    recipients.clear();
}

bool StealthcashBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    ...
    {
        int inputType; // 0 STX, 1 StealthCash
        switch(rcp.txnTypeInd)
        {
            case TXT_STX_TO_STX:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::STX, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
            case TXT_STX_TO_ANON:
                formatted.append(tr("<b>%1</b> to STEALTHCASH %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::STX, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_ANON:
                formatted.append(tr("<b>%1</b>, ring size %2 to STEALTHCASH %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::STX, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                nAnonOutputs++;
                break;
            case TXT_ANON_TO_STX:
                formatted.append(tr("<b>%1</b>, ring size %2 to STX %3 (%4)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::STX, rcp.amount), QString::number(rcp.nRingSize), Qt::escape(rcp.label), rcp.address));
                inputType = 1;
                break;
            default:
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::STX, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsStealthCashBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your stealthcash balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::STX, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void StealthcashBridge::openAddressBook(bool sending)
{
...
void StealthcashBridge::openCoinControl()
{
...
void StealthcashBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void StealthcashBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap StealthcashBridge::listAnonOutputs()
{
...
void StealthcashBridge::populateTransactionTable()
{
...
void StealthcashBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void StealthcashBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString StealthcashBridge::transactionDetails(QString txid)
{
...
// Addresses
void StealthcashBridge::populateAddressTable()
{
...
void StealthcashBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void StealthcashBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString StealthcashBridge::newAddress(bool own)
{
...
QString StealthcashBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void StealthcashBridge::updateAddressLabel(QString address, QString label)
{
...
bool StealthcashBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool StealthcashBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void StealthcashBridge::appendMessages(QString messages, bool reset)
{
    emitMessages("[" + messages + "]", reset);
}

void StealthcashBridge::appendMessage(int row) {
...
void StealthcashBridge::populateMessageTable()
{
...
void StealthcashBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool StealthcashBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool StealthcashBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString StealthcashBridge::getPubKey(QString address, QString label)
{
    if(!label.isEmpty())
...
bool StealthcashBridge::setPubKey(QString address, QString pubkey)
{
...
bool StealthcashBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
    WalletModel::UnlockContext ctx(window->walletModel->requestUnlock());
...
void StealthcashBridge::connectSignals()
{
...
QVariantMap StealthcashBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap StealthcashBridge::listLatestBlocks()
{
...
QVariantMap StealthcashBridge::findBlock(QString searchID)
{
...
QVariantMap StealthcashBridge::blockDetails(QString blkHash)
{
...
QVariantMap StealthcashBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap StealthcashBridge::txnDetails(QString blkHash, QString txnHash)
{
