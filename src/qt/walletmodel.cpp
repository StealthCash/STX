...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedBalance(0), cachedStealthCashBal(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0),
...
qint64 WalletModel::getStealthCashBalance() const
{
    return wallet->GetStealthCashBalance();
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    qint64 newStealthCashBal = getStealthCashBalance();
    qint64 newStake = getStake();
    qint64 newUnconfirmedBalance = getUnconfirmedBalance();
    qint64 newImmatureBalance = getImmatureBalance();

    if (cachedBalance != newBalance
        || cachedStealthCashBal != newStealthCashBal
        ...
        || cachedImmatureBalance != newImmatureBalance)
    {
        ...
        cachedUnconfirmedBalance = newUnconfirmedBalance;
        cachedImmatureBalance = newImmatureBalance;
        emit balanceChanged(newBalance, newStealthCashBal, newStake, newUnconfirmedBalance, newImmatureBalance);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    if (vNodes.empty())
        return SendCoinsReturn(SCR_ErrorWithMsg, 0, QString::fromStdString("StealthCash is not connected!"));
    ...
    {
        nBalance = wallet->GetStealthCashBalance();
        if ((nTotalOut + nTransactionFee) > nBalance)
            return SendCoinsReturn(SCR_AmountWithFeeExceedsStealthCashBalance, nTransactionFee);
    };
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.txnTypeInd == TXT_STX_TO_STX
                || rcp.txnTypeInd == TXT_ANON_TO_STX)
            {
                // -- out sdc
                std::string sError;
