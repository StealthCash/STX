...
var unit = {
    ...
    setType: function(type) {
        ...
        switch(type) {
            ...
            case 2:
                this.name = "uSTX",
                this.display = "&micro;STX";
                break;

            case 3:
                this.name    = "sSTX",
                this.display = "StealthCashshi";
                break;

            default:
                this.name = this.display = "STX";
        }
...
var overviewPage = {
    init: function() {
        ...
        this.stealthcashBal = $("#stealthcashBal"),
        ...
        // Announcement feed
        $.ajax({
        //    url:"http://ajax.googleapis.com/ajax/services/feed/load?v=2.0\&q=http://stealthcashhangout.com/category/18.rss",
		url:"",
            dataType: 'jsonp'
        ...
        menu = [{
                 ...
                 {
                     name: ' About&nbsp;StealthCash...',
                     img: 'qrc:///icons/stealthcash',
                     fun: function () {
    ...
    updateBalance: function(balance, stealthcashBal, stake, unconfirmed, immature) {
        if(balance == undefined)
            balance     = this.balance    .data("orig"),
            stealthcashBal   = this.stealthcashBal  .data("orig"),
            ...
            this.stealthcashBal  .data("orig", stealthcashBal),
        ...
        this.formatValue("stealthcashBal",   stealthcashBal);
    ...
    $("#recipients").append((
        ...
        +  '<div id="recipient[count]" class="recipient"> \
            <div class="flex-right"> \
                ...
                <input id="pay_to[count]" class="pay_to input_box" title="The address to send the payment to  (e.g. SXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" placeholder="Enter a StealthCash address (e.g. SXywGBZBowrppUwwNUo1GCRDTibzJi7g2M)" maxlength="128" oninput="base58.check(this);" onchange="$(\'#label[count]\').val(bridge.getAddressLabel(this.value));"/> \
            ...
            <div class="flex-left"> \
                ...
                <select id="unit[count]" class="unit button is-inverse has-fixed-icon"  style="margin-left:10px; height:43px; width:100px;" onchange="unit.format(\'#amount[count]\', $(this).val());"> \
                    <option value="0" title="StealthCash"                    ' + (unit.type == 0 ? "selected" : "") + '>STX</option> \
                    <option value="1" title="Milli-StealthCash (1 / 1000)"   ' + (unit.type == 1 ? "selected" : "") + '>mSTX</option> \
                    <option value="2" title="Micro-StealthCash (1 / 1000000)"' + (unit.type == 2 ? "selected" : "") + '>&micro;STX</option> \
                    <option value="3" title="StealthCashshi (1 / 100000000)" ' + (unit.type == 3 ? "selected" : "") + '>StealthCashshi</option> \
                </select> \
            </div> \
...
function StealthCashChatInit() {
    var menu = [{
            name: 'Send&nbsp;StealthCash',
            fun: function () {
                clearRecipients();
