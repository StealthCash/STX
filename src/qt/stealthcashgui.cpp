...
StealthcashGUI::StealthcashGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new StealthcashBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("StealthCash") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/stealthcash"));
    setWindowIcon(QIcon(":icons/stealthcash"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/stealthcash/index.html");
#else
    QFile html("/opt/stealthcash/index.html");
#endif
...
void StealthcashGUI::pageLoaded(bool ok)
{
...
void StealthcashGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);
}

void StealthcashGUI::urlClicked(const QUrl & link)
{
...
void StealthcashGUI::createActions()
{
...
void StealthcashGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void StealthcashGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("StealthCash") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/stealthcash_testnet"));
            setWindowIcon(QIcon(":icons/stealthcash_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/stealthcash_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("StealthCash client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/stealthcash_testnet"));
...
void StealthcashGUI::setWalletModel(WalletModel *walletModel)
{
...
void StealthcashGUI::setMessageModel(MessageModel *messageModel)
{
...
void StealthcashGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setIcon(QIcon(":/icons/stealthcash"));
...
#ifndef Q_OS_MAC
void StealthcashGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void StealthcashGUI::aboutClicked()
{
    AboutDialog dlg;
...
void StealthcashGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("title", tr("%n active connection(s) to StealthCash network", "", count));
}

void StealthcashGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void StealthcashGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void StealthcashGUI::changeEvent(QEvent *e)
{
...
void StealthcashGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void StealthcashGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::STX, nFeeRequired));
...
void StealthcashGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void StealthcashGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void StealthcashGUI::gotoSignMessageTab(QString addr)
{
...
void StealthcashGUI::gotoVerifyMessageTab(QString addr)
{
...
void StealthcashGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void StealthcashGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void StealthcashGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void StealthcashGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
void StealthcashGUI::handleURI(QString strURI)
{
...
void StealthcashGUI::setEncryptionStatus(int status)
{
...
void StealthcashGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void StealthcashGUI::backupWallet()
{
...
void StealthcashGUI::changePassphrase()
{
...
void StealthcashGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void StealthcashGUI::lockWallet()
{
    if(!walletModel)
        return;
...
void StealthcashGUI::toggleLock()
{
    if(!walletModel)
        return;
...
void StealthcashGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void StealthcashGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void StealthcashGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void StealthcashGUI::updateStakingIcon()
{
...
void StealthcashGUI::detectShutdown()
{
