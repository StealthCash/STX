...
bool CBlock::AcceptBlock()
{
    ...
    if (Params().IsProtocolVStart(nHeight) && nVersion < 7)
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    if (Params().IsProtocolVStart(nBestHeight+1))
    ...
    {
        int64_t nSearchInterval = Params().IsProtocolVStart(nBestHeight+1) ? 1 : nSearchTime - nLastCoinStakeSearchTime;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        LogPrintf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "StealthCash", CClientUIInterface::BTN_OK | CClientUIInterface::ICON_WARNING | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    };
    return true;
